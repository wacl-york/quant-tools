...
# Odd NO2 data, very low measurements then sudden step change ramps up around winter
df_clean %>%
    ...
    ggplot(aes(x=timestamp, y=value)) +
        ...
        labs(x="", y="Pollutant (ugm-3 ppb)", title="Vortex out of the box (24 hour average)")

# Save!
setcolorder(df_clean, c("timestamp", "manufacturer", "device", "measurand", "value"))
df_clean <- df_clean[ !is.na(value) ]
write_csv(df_clean, "Data/Vortex.csv")
